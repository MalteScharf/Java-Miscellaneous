...
public class Woerterraten {
    ...
    void aktualisiereLoesung(char c) {
        // String Buffer initialisieren
        StringBuilder sbZuRaten = new StringBuilder(this.zuRatendesWort);
        for (int i = 0; i< sbZuRaten.length(); i++){
            // Wenn c in zuRatendesWort, dann aktualisiere erratenens Wort
            if (c == sbZuRaten.charAt(i)){
                erratenesWort.replace(i,i+1,Character.toString(c));
            }
        }
